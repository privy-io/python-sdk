# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from typing import Optional
from typing_extensions import Literal

from .._models import BaseModel
from .curve_signing_chain_type import CurveSigningChainType
from .embedded_wallet_recovery_method import EmbeddedWalletRecoveryMethod

__all__ = ["LinkedAccountCurveSigningEmbeddedWallet"]


class LinkedAccountCurveSigningEmbeddedWallet(BaseModel):
    """A curve signing embedded wallet account linked to the user."""

    id: Optional[str] = None

    address: str

    chain_id: str

    chain_type: CurveSigningChainType
    """The wallet chain types that support curve-based signing."""

    connector_type: Literal["embedded"]

    delegated: bool

    first_verified_at: Optional[float] = None

    imported: bool

    latest_verified_at: Optional[float] = None

    public_key: str

    recovery_method: EmbeddedWalletRecoveryMethod
    """The method used to recover an embedded wallet account."""

    type: Literal["wallet"]

    verified_at: float

    wallet_client: Literal["privy"]

    wallet_client_type: Literal["privy"]

    wallet_index: float

    user_can_sign: Optional[bool] = None
    """Whether the user can unilaterally sign with this wallet."""
